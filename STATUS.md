# KongTradeBot Live Status

> 2026-05-01 10:25:15 UTC

## Bot-Status
```
RUNNING (seit Thu 2026-04-30 09:17:51 UTC)
```

## Dashboard-URL
```
https://judges-stanford-candidate-classifieds.trycloudflare.com
```

## Watchdog
```
OK — lief vor 29s | HB: OK (354s alt)
```

## Portfolio
```
Total:         $4.62 USDC
Cash:          $0.00 USDC
In Positionen: $4.62 USDC
Offene Pos.:   0
PnL heute:     $0.00
```

## Letzte Trades (5)
```
2026-05-01 08:31:16 | INFO     | polymarket_bot.wallet_monitor | 🆕 NEUER TRADE erkannt: TradeSignal(BUY Yes @ $0.074 | $7.40 USDC | Will Portugal win the 2026 FIFA World Cup?)
2026-05-01 08:31:16 | INFO     | polymarket_bot.wallet_monitor | 🆕 NEUER TRADE erkannt: TradeSignal(BUY Yes @ $0.074 | $51.80 USDC | Will Portugal win the 2026 FIFA World Cup?)
2026-05-01 08:31:16 | INFO     | polymarket_bot.wallet_monitor | 🆕 NEUER TRADE erkannt: TradeSignal(BUY Yes @ $0.074 | $15.00 USDC | Will Portugal win the 2026 FIFA World Cup?)
2026-05-01 08:31:16 | INFO     | polymarket_bot.wallet_monitor | 🆕 NEUER TRADE erkannt: TradeSignal(BUY Yes @ $0.153 | $38.96 USDC | Will Spain win the 2026 FIFA World Cup?)
2026-05-01 08:31:16 | INFO     | polymarket_bot.wallet_monitor | 🆕 NEUER TRADE erkannt: TradeSignal(BUY Yes @ $0.153 | $10.00 USDC | Will Spain win the 2026 FIFA World Cup?)
```

## Letzte Fehler (3)
```
[py_clob_client_v2] request error status=400 url=https://clob.polymarket.com/v1/heartbeats body={"heartbeat_id":"e08bd482-849c-438d-b0a7-07d6aaa56c42","error_msg":"Invalid Heartbeat ID"}
2026-05-01 10:20:04 | WARNING  | polymarket_bot.execution | [V2] heartbeat failed: PolyApiException[status_code=400, error_message={'heartbeat_id': 'e08bd482-849c-438d-b0a7-07d6aaa56c42', 'error_msg': 'Invalid Heartbeat ID'}]
[py_clob_client_v2] request error status=400 url=https://clob.polymarket.com/v1/heartbeats body={"heartbeat_id":"97b9702e-a2ed-47c1-beaa-7c0be8189ce8","error_msg":"Invalid Heartbeat ID"}
```

---
*Naechstes Update in ~5 Minuten*
